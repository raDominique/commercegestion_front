...
export default function ActifsTable({ loading, actifs, dateFormat, isDesktop, onShowDetail, onOpenStockModal, onOpenSellModal, onVirerDroit }) {
  ...
  const handleActionClick = async (actionType, itemId, actionFunction, item) => {
    ...
    try {
      if (actionFunction) {
        await actionFunction(item || itemId);
      }
    } catch (error) {
      console.error(`Error during ${actionType} action for item ${itemId}:`, error);
    } finally {
      setLoadingStates(prev => ({ ...prev, [`${actionType}-${itemId}`]: false }));
    }
  };
        ...
        <Table>
          ...
          <TableBody>
            {actifs.map(item => {
              ...
              return (
                <TableRow key={itemId || item.productId || JSON.stringify(item)}>
                      ...
                      <div className="flex gap-2 justify-end">
                        {onShowDetail && (
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => handleActionClick('showDetail', itemId, onShowDetail, item)}
                            loading={loadingStates[`showDetail-${itemId}`]}
                          >
                        ...
                        {onOpenStockModal && (
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => handleActionClick('openStockModal', itemId, onOpenStockModal, item)}
                            loading={loadingStates[`openStockModal-${itemId}`]}
                          >
                        ...
                        {onOpenSellModal && (
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => handleActionClick('openSellModal', itemId, onOpenSellModal, item)}
                            loading={loadingStates[`openSellModal-${itemId}`]}
                          >
                      ...
                      {onVirerDroit && (
                        <div>
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => handleActionClick('virerDroit', itemId, onVirerDroit, item)}
                            loading={loadingStates[`virerDroit-${itemId}`]}
                          >
